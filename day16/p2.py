...
class Maze:
    ...
    def print_maze_with_paths(self, paths):
        maze = self.maze.copy()
        for path in paths:
            for x, y in path.get_path_to_root():
                maze[y][x] = "O"
        for line in maze:
            print("".join(line))
    ...
    def find_optimal_path3(self):
        root = TreeNode(self.start, START_DIRECTION, 1)
        cheapest_path = None
        queue = deque([root])
        visited_states = {(self.start, START_DIRECTION): 1}
        best_paths = []
            ...
            for move in moves:
                ...
                if new_pos == self.end:
                    if not cheapest_path or child.cost < cheapest_path.cost:
                        best_paths = []
                        cheapest_path = child
                    if cheapest_path.cost == child.cost:
                        best_paths.append(child)
                    continue
                visited_states[new_state] = child.cost
                queue.append(child)
        ...
        for path in best_paths:
            path_to_root = path.get_path_to_root()
            nodes |= set(path.get_path_to_root())
        print(f"Tile count: {len(nodes)}")
        pprint(cheapest_path.cost)
        self.print_maze_with_paths(best_paths)
